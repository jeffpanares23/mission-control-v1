...
class AnniversaryController extends BaseApiController
{
    public function index(Request $request)
    {
        ...
        $result = $this->supabase->get('anniversaries', ['user_id' => "eq.{$userId}", 'order' => 'anniversary_date.asc']);
    ...
    {
        $result = $this->supabase->update('anniversaries', ['id' => $id], $request->only([
            'title','anniversary_date','anniversary_type','notes','remind_days_before','is_recurring','account_id','metadata'
        ]));
    ...
    {
        $result = $this->supabase->delete('anniversaries', ['id' => $id]);
