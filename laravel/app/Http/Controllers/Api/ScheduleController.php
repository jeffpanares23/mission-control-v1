...
class ScheduleController extends BaseApiController
{
    public function index(Request $request)
    {
        ...
        $result = $this->db($request)->get('schedules', [
            'user_id' => "eq.{$userId}",
    ...
    {
        $result = $this->db($request)->update('schedules', ['id' => $id], $request->only([
    ...
    {
        $result = $this->db($request)->delete('schedules', ['id' => $id]);
