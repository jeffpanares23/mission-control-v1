...
class DashboardController extends BaseApiController
{
    ...
    public function index(Request $request)
    {
        ...
        $tasks = $this->supabase->get('tasks', ['user_id' => "eq.{$userId}", 'order' => 'created_at.desc', 'limit' => 100]);
        $accounts = $this->supabase->get('accounts', ['user_id' => "eq.{$userId}"]);
        $insights = $this->supabase->get('insights', ['user_id' => "eq.{$userId}", 'is_dismissed' => 'eq.false', 'order' => 'created_at.desc', 'limit' => 10]);
        $upcoming = $this->supabase->get('schedules', ['user_id' => "eq.{$userId}", 'start_time' => 'gte.now()', 'order' => 'start_time.asc', 'limit' => 5]);
    ...
    public function activity(Request $request)
    {
        ...
        $log = $this->supabase->get('activity_log', [
            'user_id' => "eq.{$userId}",
